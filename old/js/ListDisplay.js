...
export class ListView {
    constructor(root_id, {sorters, filters, makenewitem, update, modallogic}) {
        this.rootnode = document.getElementById(root_id)
        ...
        this.update_list = update
        this.sorters = sorters
        this.modal_logic = modallogic
    }

    get_last_visible_element() {
        let l = this.display_tasks[this.display_tasks?.length - 1]
        return this.all_views[l?.id]
    }
    ...
    async change_filtering(newfiltering, searchterm = "") {
        ...
        this.current_search_term = searchterm
        this.current_filter = newfiltering
        this.rootnode.firstElementChild.scrollIntoView({behavior: 'smooth'})
        await this.view_refresh()
    }
    ...
    async view_refresh() {
        ...
        this.all_tasks.forEach((task, i) => {
            ...
            if (added) {
                ...
                taskitem.elem.classList.add('visible')
        ...
        let lastadded = toadd[toadd.length-1]
        if (lastadded) {
            setTimeout(() => {
                lastadded.elem.scrollIntoView({'behavior': 'smooth'})
            }, 550);
        } else {
            setTimeout(() => {
                this.rootnode.parentElement.scrollTop = this.rootnode.parentElement.scrollHeight
                console.log('rooty is ', this.rootnode)
            }, 550);
            // await transition_stop(this.get_last_visible_element())
            // this.rootnode.firstElementChild.scrollIntoView({behavior: 'smooth'})
            // setTimeout(() => {
            //     this.get_last_visible_element().elem.scrollIntoView({behavior: 'smooth'})
            // }, 550);
        }

        // let last_displayed = this.display_tasks.slice(-1)[0]
        // if (last_displayed) {
        //     await transition_stop(this.all_views[last_displayed.id].elem)
        // }
    ...
    async view_filtersort() {
        ...
        this.all_tasks = Object.values(api.tasklist).sort(sorters[this.current_sort])
        // If we want to search, then we need to add a search filter
        // Then, filter the sorted tasks, and cache that result
        this.display_tasks = this.all_tasks.filter(this.filters[this.current_filter])
        // Chin links ahoy!
        return this
    }
